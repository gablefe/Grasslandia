...
data_CH1 <- data_wide %>% select(Date, CH1, IN1)  
data_CH1
data_CH1 <- gather(data_CH1, Chamber, Temperature, CH1:IN1, factor_key=TRUE) 
#gives warning: attributes are not identical across measure variables; they will be dropped 
data_CH1
...
sumDat_CH1 #gives same value for every day
...
sumDat_CH1 #1: why did it re-sort table after I had gathered data_long above to exclude 
